...
def parseGPS():
    ...
    if readFix != 0:
        ...
        GPSdata['date'] = myDate
        # Store altitude in GPSdata dictionary
        GPSdata['alt'] = NMEAmain['GPGGA'].split(',')[9]


def dispOLED():
    global system_state, latitude_1, longitude_1, latitude_2, longitude_2
    dsp.fill(0)
    ...
    if GPSdata['fix'] == True:
        ...
        if system_state % 5 == 1:
            latitude_1 = GPSdata['latDD']
            longitude_1 = GPSdata['lonDD']
            system_state = 2
        if system_state % 5 == 2:
            dsp.fill(0)
            dsp.text("Ultimate GPS...", 0, 0)
            dsp.text(GPSdata['date'][0:5] + ' ' + GPSdata['time'], 0, 16)
            dsp.text("P1 Lat: " + str(latitude_1), 0, 26)
            dsp.text("P2 Long: " + str(longitude_1), 0, 36)
            dsp.text("Proceed to P2", 0, 46)
            dsp.text("Then Press Btn", 0, 56)
        if system_state % 5 == 3:
            latitude_2 = GPSdata['latDD']
            longitude_2 = GPSdata['lonDD']
            system_state = 4
        if system_state % 5 == 4:
            dsp.fill(0)
            dsp.text("Ultimate GPS...", 0, 0)
            dsp.text(GPSdata['date'][0:5] + ' ' + GPSdata['time'], 0, 16)
            dist = calculate_distance(latitude_1, longitude_1, latitude_2, longitude_2)
            heading = calculate_heading(latitude_1, longitude_1, latitude_2, longitude_2)
            dsp.text("Distance: " + str(dist), 0, 26)
            dsp.text("Heading: " + str(heading), 0, 36)
    dsp.show()


def button_one_irq(pin):
    global button_one_up, button_one_down, button_one_old, system_state
    button_one_value = button_one.value()
    if button_one_value == 0:
        button_one_down = time.ticks_ms()
    if button_one_value == 1:
        button_one_up = time.ticks_ms()
    if button_one_old == 1 and button_one_value == 0 and button_one_down - button_one_up > 50:
        system_state += 1
        print("Button One Triggered...")
    button_one_old = button_one_value


button_one.irq(trigger=Pin.IRQ_FAILING | Pin.IRQ_RISING, handler=button_one_irq)
